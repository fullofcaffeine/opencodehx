...
- Temperature, `topP`, `topK`, max-output-token cap/fallback helpers now shared by session compaction, upstream `smallOptions` defaults, and reasoning variant generation across the main upstream provider families, including Grok mini special cases, Gateway Google/generic routing, and newer Copilot GPT-5 release-date `xhigh` behavior.
