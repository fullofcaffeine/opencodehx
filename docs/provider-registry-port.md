...
- `CliSmoke` and `scripts/harness/cli-smoke.mjs` run `run --live-ai-sdk --model local-live/chat` against a local OpenAI-compatible SSE server, proving the generated CLI can stream a successful assistant response through the real AI SDK facade while asserting the request URL, bearer header, and streaming request flag.
