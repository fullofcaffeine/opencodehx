...
Remaining provider-style output polish is limited to broader generic cases: `Map.exists(...)` followed by `Map.get(...)`, key-iteration `Map.get(...)` handoffs, and verbose large object-construction locals. Those are tracked by Genes `genes-cka` and should stay compiler-generic rather than changing the typed Haxe provider registry model.
