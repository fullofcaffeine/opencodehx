...
- `CliSmoke` and `scripts/harness/cli-smoke.mjs` run explicit non-fake `run --model local-live/chat` and plain config-model `run` paths against local OpenAI-compatible success, tool-call, and failure endpoints, proving the generated CLI can stream a successful assistant response, execute model-emitted `read` and `write` tool calls with continuation requests, verify `write` file content, or surface provider-error events through the real AI SDK facade while asserting the request URL, bearer header, streaming request flag, and persisted assistant `finish` metadata.
