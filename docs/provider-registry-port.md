...
- Temperature, `topP`, `topK`, max-output-token cap/fallback helpers now shared by session compaction, upstream `smallOptions` defaults, GPT-5 verbosity/reasoning defaults, Google/Vertex thinkingConfig gating, and reasoning variant generation across the main upstream provider families, including generic reasoning-effort provider families, Grok mini special cases, Gateway Google/generic routing, Azure/OpenAI exclusions, and newer Copilot GPT-5 release-date `xhigh` behavior.
