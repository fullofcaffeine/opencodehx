...
Remaining provider-style output polish is now limited to verbose large object-construction locals such as single-use `parsedN` temporaries around typed record construction and patch helper calls. That is tracked by OpenCodeHX `opencodehx-mlv` and Genes `genes-s5f` and should stay compiler-generic rather than changing the typed Haxe provider registry model.
