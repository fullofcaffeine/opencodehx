...
- `CliSmoke` and `scripts/harness/cli-smoke.mjs` run explicit non-fake `run --model local-live/chat` and config-model `run --live-ai-sdk` paths against local OpenAI-compatible success and failure endpoints, proving the generated CLI can stream a successful assistant response or surface provider-error events through the real AI SDK facade while asserting the request URL, bearer header, streaming request flag, and persisted assistant `finish` metadata.
