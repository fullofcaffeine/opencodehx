...
**Beads:** `opencodehx-024`, `opencodehx-025`, `opencodehx-nrh`, `opencodehx-hup`, `opencodehx-7aan`, `opencodehx-acxa`, `opencodehx-24ys`, `opencodehx-g8sn`, `opencodehx-000.3.2`
...
- `opencodehx.auth.AuthStore` owns the Node auth storage seam for `OPENCODE_AUTH_CONTENT` and XDG data `auth.json`, validating upstream `api`, `oauth`, and `wellknown` entry shapes through `genes.ts.UnknownRecord`/`UnknownNarrow` before provider/config code sees them. Its set/remove path normalizes trailing slash URL keys the same way upstream `auth/auth.test.ts` expects.
...
- `CliSmoke` and `scripts/harness/cli-smoke.mjs` run explicit non-fake `run --model local-live/chat` and plain config-model `run` paths against local OpenAI-compatible success, tool-call, and failure endpoints, proving the generated CLI can stream a successful assistant response, execute model-emitted `read`, `write`, `edit`, `apply_patch`, and `bash` tool calls with continuation requests, verify write/edit/patch/bash side effects, or surface provider-error events through the real AI SDK facade while asserting the request URL, bearer header, streaming request flag, and persisted assistant `finish` metadata.
- `SessionProcessorSmoke` proves live `SessionProcessor.runAiSdk` request assembly reaches `streamText(...)` by inspecting `MockLanguageModelV3.doStreamCalls`: max output tokens, temperature, `topP`, `topK`, request headers, transformed provider options, variant-selected options, selected-agent prompt/options/temperature/tool filtering, disabled-tool execution rejection, and the same option set on tool-result continuation calls. It also verifies recovered user file parts and assistant tool-call/tool-result history in the converted provider prompt.
- `SessionProcessorSmoke` proves the live AI SDK middleware calls `ProviderTransform.message(...)` before `doStream`, including Anthropic empty-system filtering and recovered media tool-result history at the inspected provider prompt boundary.
...
- Simple bundled-provider no-network factory/model selection through `@ai-sdk/mistral`, `@ai-sdk/groq`, `@ai-sdk/cohere`, `@ai-sdk/perplexity`, `@ai-sdk/deepinfra`, `@ai-sdk/cerebras`, `@ai-sdk/gateway`, `@ai-sdk/togetherai`, `@ai-sdk/vercel`, and `@ai-sdk/alibaba`, forwarding only the stable `baseURL`, `apiKey`, and header settings until typed fetch, ID-generation, metadata-cache, embedding/video endpoint, and provider-specific request-option seams own the remaining SDK hooks.
- Venice no-network factory/model selection through `venice-ai-sdk-provider@2.0.2`, the AI SDK v6-compatible package line, including `name`, `baseURL`, API key, provider/model headers, `includeUsage`, and structured-output support narrowed into `VeniceProviderSettings`.
- OpenRouter no-network factory/model selection through `@openrouter/ai-sdk-provider`, including `baseURL`, API key, provider/model headers, compatibility mode, `extraBody`, BYOK `api_keys`, and app attribution settings narrowed from provider options into the SDK settings bridge.
...
- Temperature, `topP`, `topK`, max-output-token cap/fallback helpers now shared by session compaction, upstream `smallOptions` defaults, GPT-5 verbosity/reasoning defaults, Google/Vertex thinkingConfig gating, and reasoning variant generation across the main upstream provider families, including generic reasoning-effort provider families, DeepSeek/Minimax/GLM/Cohere/Perplexity exclusions, Anthropic/Bedrock budget and adaptive variants, SAP Anthropic/Gemini/GPT/o-series routing, Grok mini special cases, Gateway Google/generic routing, Gateway Anthropic dot-format IDs, Azure/OpenAI exclusions, OpenAI release-date effort expansion, and newer Copilot GPT-5/codex `xhigh` behavior.
...
The map facade leak is fixed in `../genes` commit `57fa1e6ad6419423d905a6825fc2c91d3a37b6b6` (`genes-oih`): `genes.util.EsMap` facade methods now stay non-inline, so provider generated output calls stable public APIs such as `providers.get(...)`, `models.set(...)`, and `provider.models.keys()` instead of exposing the backing native `Map` field.

The models.dev normalization loop then exposed a generic same-block null-flow gap: after `final modelData = provider.models.get(id); if (modelData == null) continue;`, generated TS still asserted or cast `modelData` later in the same loop body. The fix landed in `../genes` commit `0897b1a7af382ea5dcb887648eeaf0c99ce396d9` (`genes-rlr`), teaching `genes-ts` to carry non-null facts after exiting guards such as `continue`, `break`, `return`, and `throw`, while resetting those facts inside nested function expressions. Provider generated output now uses guarded `modelData` and `modeData` directly in the models.dev path.

The remaining provider map handoffs were fixed in `../genes` commit `c4cf03c7cb614cefe4b1bab169ca44a67a19c828` (`genes-cka`): stable `Map.exists(key)` checks and `Map.keys()` iteration facts now narrow following same-map `Map.get(key)` reads for non-null value types. Generated provider output no longer needs broad `Register.unsafeCast` calls for those existence/key-iteration paths; strict TS gets direct reads or precise non-null assertions where TypeScript cannot infer Haxe map semantics.

The remaining provider object-construction temporary names were fixed in `../genes` commit `84952a73b9978fa3d47ce213d992a6b2e65ec3d8` (`genes-s5f`): Haxe-lowered same-prefix record-construction temps keep their separate declarations to preserve evaluation order, but generated TS now uses object-field names such as `family`, `status`, `capabilities`, `url`, `npm`, and `variants` instead of `parsedN` / `baseN` names. The Haxe provider model stays unchanged.
