...
- NPM/install-adjacent behavior:
  ...
  - upgrade command planning covers curl install-script fetch/stdin/env/version-probe behavior, npm/pnpm/Bun/Scoop command/version-probe behavior, Homebrew core upgrade, tap refresh/upgrade, tap failure containment, and tap pull failure containment, Chocolatey upgrade plus elevated-shell failure messaging, and unknown-method no-command failure,
