...
- NPM/install-adjacent behavior:
  ...
  - package `install` skips non-writable directories through injected and default filesystem writability checks, treats clean dependency/devDependency/peerDependency/optionalDependency and requested package-lock roots as current, reifies missing requested packages with their add specs, reifies missing `node_modules` with no-input, versioned, and bare add specs, and reifies missing, malformed, or dirty package-lock roots,
  ...
  - package `outdated` covers registry failure, malformed registry JSON, missing latest metadata, exact-version comparison including build metadata and `v`-prefixed latest versions, comparator edge satisfaction/escape, whole-range plus minor/patch wildcard satisfaction/escape, zero-major caret, tilde, and disjunction semver ranges,
  ...
  - latest-version lookup covers unknown-method GitHub fallback, GitHub releases, npm/pnpm/Bun registry behavior with registry URL normalization, Homebrew core/tap, Scoop, and Chocolatey response shapes,
...
`FileWatcherRuntime` is intentionally narrow: it converts native Node `fs.watch` callbacks into typed `FileUpdatedEvent` records, publishes deterministic root add/change/unlink events through an injected backend, suppresses `.git/index`, and filters git metadata notifications down to `.git/HEAD` for VCS branch refresh. The injected smoke backend keeps normal `npm run smoke` deterministic, while `npm run file:watcher:smoke` gives opt-in native watcher evidence on hosts where `fs.watch` is reliable.
