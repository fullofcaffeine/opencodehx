...
- NPM/install-adjacent behavior:
  ...
  - upgrade command planning covers curl install-script fetch/stdin/env/version-probe behavior, npm/pnpm/Bun global install commands, Homebrew core upgrade and tap refresh/upgrade, Scoop install command, Chocolatey elevated-shell failure messaging, and unknown-method no-command failure,
