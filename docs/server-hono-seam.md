...
**Beads:** `opencodehx-026`, `opencodehx-027`, `opencodehx-jr4`, `opencodehx-x4i`, `opencodehx-o48`, `opencodehx-p12e`, `opencodehx-3eee`
...
- `opencodehx.server.OpenCodeServer` exposes a first route set: `/health`, `/event`, `/permission` GET/reply, `/question` GET/reply/reject, `/session` GET/POST, `/session/status`, `/session/:sessionID` GET/PATCH for the title/archive subset, `/session/:sessionID/children`, `/session/:sessionID/message`, `/session/:sessionID/abort`, `/project/current`, `/project/git/init`, `/sync/start`, `/sync/replay`, `/sync/history`, `/tui/select-session`, `/pty`, `/pty/:ptyID`, and `/pty/:ptyID/connect`.
