...
**Beads:** `opencodehx-026`, `opencodehx-027`, `opencodehx-jr4`, `opencodehx-x4i`, `opencodehx-o48`, `opencodehx-p12e`, `opencodehx-3eee`, `opencodehx-tg29`, `opencodehx-mi6i`, `opencodehx-4pth`, `opencodehx-c7jc`
...
- `opencodehx.server.OpenCodeServer` exposes a first route set: `/health`, `/config`, `/event`, `/permission` GET/reply, `/question` GET/reply/reject, `/session` GET/POST, `/session/status`, `/session/:sessionID` GET/PATCH/DELETE for the title/archive/delete subset, `/session/:sessionID/children`, `/session/:sessionID/message`, `/session/:sessionID/message/:messageID` GET/DELETE, `/session/:sessionID/abort`, `/project`, `/project/current`, `/project/git/init`, `/sync/start`, `/sync/replay`, `/sync/history`, `/tui/select-session`, `/pty`, `/pty/:ptyID`, and `/pty/:ptyID/connect`.
...
- `opencodehx.smoke.ServerSmoke` covers in-memory `app.request()` routes, `/config` local config output with upstream JSON keys, `/permission` list/reply with invalid-body handling, unknown request no-op, `once`/`always`/`reject` behavior, and routed-directory isolation, `/question` list/reply/reject with routed-directory isolation, `/session` list filtering by directory/root/start/search/limit, `/session/:id` detail reads, `/session/:id/children` child listing and missing-parent errors, `/session/:id` deletion with child/message cascade and list pruning, `/session/:id/message` pagination plus `/session/:id/message/:messageID` detail/delete behavior, `/session/status` returning the upstream-shaped empty active-status record after synchronous completion, injected live AI SDK `/session` execution with an observable busy status while the stream is active, active live abort routing with idle cleanup and persisted `MessageAbortedError` state, config-backed live provider/default-agent selection reaching a local no-network AI SDK request, `/experimental/session` global listing across routed projects with project metadata/cursor/search/archive behavior, `/project`, `/project/current`, and `/project/git/init` behavior including multi-project list output, git initialization/reload behavior, and server-attached snapshot tracking after reload, create routing through `x-opencode-directory`, optional child-session `parentID`, SSE text/status/idle emission, message cursor headers, bad/missing session cases, upstream's `before`-requires-`limit` validation edge, high-volume legacy message limits, select-session validation, abort success and missing-session abort validation, PTY HTTP routes, listener start/stop, and a real PTY WebSocket write/replay/tail flow.
