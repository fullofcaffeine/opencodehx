...
- `OpenCodeServer` can also receive an injected `ServerLiveAiSdkOptions` runtime for credential-free server-owned live session evidence. This keeps default `/session` deterministic while proving the server can await `SessionProcessor.runAiSdk`, persist the turn, expose active status during the async stream, and abort an active not-yet-persisted stream through the server-owned `AbortSignal`.
