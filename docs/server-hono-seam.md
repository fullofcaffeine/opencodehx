...
This is not full server parity yet. Remaining work includes Bus/AsyncQueue-backed events, OpenAPI middleware, request validation/error taxonomy, full published SDK package compatibility, real session prompt/action routes, provider streams, auth/CORS/compression, server-attached Snapshot service behavior after project git init, full workspace routing/control-plane service integration, and Bun adapter parity.
