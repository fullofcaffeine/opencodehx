...
- `opencodehx.server.OpenCodeServer` exposes a first route set: `/health`, `/event`, `/session` GET/POST, `/session/status`, `/session/:sessionID` PATCH for the title/archive subset, `/session/:sessionID/message`, `/session/:sessionID/abort`, `/project/current`, `/project/git/init`, `/sync/start`, `/sync/replay`, `/sync/history`, `/tui/select-session`, `/pty`, `/pty/:ptyID`, and `/pty/:ptyID/connect`.
