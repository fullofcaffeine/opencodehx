...
- `opencodehx.server.OpenCodeServer` exposes a first route set: `/health`, `/config`, `/event`, `/permission` GET/reply, `/question` GET/reply/reject, `/session` GET/POST, `/session/status`, `/session/:sessionID` GET/PATCH/DELETE for the title/archive/delete subset, `/session/:sessionID/children`, `/session/:sessionID/message`, `/session/:sessionID/message/:messageID` GET/DELETE, `/session/:sessionID/abort`, `/project`, `/project/current`, `/project/git/init`, `/project/:projectID` PATCH, `/sync/start`, `/sync/replay`, `/sync/history`, `/tui/select-session`, `/pty`, `/pty/:ptyID`, and `/pty/:ptyID/connect`.
...
- `opencodehx.smoke.ServerSmoke` covers in-memory `app.request()` routes:
  - config, permission, and question routes, including invalid-body handling, unknown request no-op behavior, `once`/`always`/`reject` decisions, and routed-directory isolation.
