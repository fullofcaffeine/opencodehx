...
- `opencodehx.server.OpenCodeServer` exposes a first route set: `/health`, `/event`, `/session` GET/POST, `/session/:sessionID` PATCH for the title/archive subset, `/session/:sessionID/message`, `/session/:sessionID/abort`, `/project/current`, `/project/git/init`, `/sync/start`, `/sync/replay`, `/sync/history`, `/tui/select-session`, `/pty`, `/pty/:ptyID`, and `/pty/:ptyID/connect`.
...
- `opencodehx.smoke.ServerSmoke` covers in-memory `app.request()` routes, `/session` list filtering by directory/root/start/search/limit, `/experimental/session` global listing across routed projects with project metadata/cursor/search/archive behavior, `/project/git/init` and `/project/current` git initialization/reload behavior including server-attached snapshot tracking after reload, create routing through `x-opencode-directory`, optional child-session `parentID`, SSE text emission, message cursor headers, bad/missing session cases, upstream's `before`-requires-`limit` validation edge, high-volume legacy message limits, select-session validation, abort success, PTY HTTP routes, listener start/stop, and a real PTY WebSocket write/replay/tail flow.
