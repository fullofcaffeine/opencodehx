...
- `StorageSmoke.jsonMigration` covers project filename ID precedence, session directory/filename ID precedence, message filename ID precedence, part filename/message-path precedence, legacy parts without `sessionID`, orphan session skipping, idempotent reruns, missing storage directory empty stats, valid todo counting with source-position preservation and invalid-entry skipping, permission file counting, session-share counting, and orphan side-table skipping.
...
This is not the full OpenCode storage service. Deferred work includes Drizzle migration compatibility, sync-event projector wiring, transaction/effect side effects, concurrent update serialization under the real Effect service, full project table parity, persisted todo/session_entry/permission/session-share tables, and JSON migration unreadable-file error collection.
