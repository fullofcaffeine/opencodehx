...
**Beads:** `opencodehx-014`, `opencodehx-e6g6`, `opencodehx-0hmi`, `opencodehx-c6qv`, `opencodehx-gggg`
...
- `opencodehx.storage.StorageJsonRuntime` models the generic upstream `Storage.Service` key/value surface over JSON files. It stores only `genes.ts.JsonValue`, maps keys to nested `.json` paths, throws `NotFound` for missing reads/updates, overwrites existing values, removes missing keys as a no-op, recursively lists sorted keys under a prefix while ignoring stray non-JSON artifacts, and serializes Promise-backed updates per key.
...
- Session info records now type stored summary diffs as `SnapshotFileDiff[]` and stored permission state as `PermissionRule[]`, matching the upstream `Snapshot.FileDiff[]` and `Permission.Ruleset` shapes while keeping SQLite JSON serialization at the store boundary.
...
- `getMessage` and `pageMessages` implement typed message hydration, upstream-style newest-first page selection, chronological page output, opaque cursor encoding, and part hydration.
- `opencodehx.session.SessionExport` builds the upstream CLI export payload shape from typed Haxe DTOs, including optional sanitization for sensitive transcript text and session path/title fields. Optional export fields use `genes.ts.Undefinable` so the serialized JSON keeps upstream's omitted-field shape instead of emitting `null`.
