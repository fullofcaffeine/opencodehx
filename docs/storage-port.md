...
This is not the full OpenCode storage service. Deferred work includes Drizzle migration compatibility, sync-event projector wiring, transaction/effect side effects, concurrent update serialization under the real Effect service, full project table parity, todo/session_entry/permission/session-share tables, and JSON migration unreadable-file error collection.
