...
- `opencodehx.lsp.LspRuntime` models enabled/disabled LSP configuration, inside-workspace checks, TypeScript extension matching, Deno marker exclusion, lazy client spawn, broken-server suppression, idempotent init, status, diagnostics aggregation, empty no-client results, and shutdown.
...
JSON-RPC request and response payloads are kept as `genes.ts.Unknown` at the protocol boundary. Domain-owned values such as diagnostics, locations, statuses, server definitions, and tool arguments are typed before storage or use.
