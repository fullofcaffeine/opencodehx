...
**Bead:** `opencodehx-016`  
**Upstream oracle:** `../opencode/packages/opencode/src/tool/tool.ts`, `registry.ts`, `invalid.ts`, `glob.ts`, `grep.ts`, `question.ts`, `skill.ts`, `webfetch.ts`, and `../opencode/packages/opencode/test/tool/{registry,glob,grep,question,skill,webfetch}.test.ts`
...
- Initial `invalid`, `glob`, and `grep` tool definitions, including typed permission request metadata and external-directory prompts for search tools.
...
- `.opencode/tool` and `.opencode/tools` custom file discovery for `.js`/`.ts` modules, deriving default-export ids from filenames without importing the module yet.
- `ToolSmoke` coverage for builtin ids, schema fields, custom tool discovery in singular/plural directories, dependency-importing custom tool files that do not crash discovery, unknown/disabled failures, invalid argument text, glob directory rejection, glob file matching, glob/grep permission request and denial shape, glob/grep external-directory request and denial shape, grep directory/file search, no-match output, question ask/reply output, skill content/file output, webfetch text/SVG/image handling, and `ToolDefinition` fresh-init behavior.

## Deliberate Boundaries

This is not the full upstream Effect/Zod/plugin registry. Dynamic description hooks, provider/model filtering, plugin-defined tools, custom tool module imports/exports/execution, full `Tool.define` Effect/Zod wrapping, truncation service file-spill/cleanup behavior, LSP/MCP tools, bash/PTY, and task/subagent descriptions are deferred to their owning beads. `QuestionTool`, `SkillTool`, and `WebFetchTool` are async and remain outside the synchronous registry/session tool loop until async tool execution lands. Core file tool permissions and read/write/edit/apply_patch scaffolding live in `docs/core-file-tools-port.md`.
