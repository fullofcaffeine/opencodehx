...
- `run --live-ai-sdk --model provider/model` is an opt-in live path through `ProviderRegistry.getLanguage` and the real AI SDK stream facade. It loads well-known remote configs from `wellknown` auth entries, the XDG global config directory, project config for the run directory, `OPENCODE_AUTH_CONTENT`, the upstream-shaped XDG data `auth.json` file, and read-only active-account `/api/config` from the upstream-shaped SQLite account tables before resolving providers. `CliSmoke` and `scripts/harness/cli-smoke.mjs` now also run a local no-network OpenAI-compatible SSE server and verify a successful streamed assistant response, request URL, authorization header, and `stream: true` request body.
