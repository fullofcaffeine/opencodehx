...
- `run --mock-ai-sdk` routes through the credential-free async AI SDK-backed session harness (`Cli.runAsync` -> `SessionProcessor.runAiSdk`). This proves the generated CLI can wait for provider promises and emit a normal transcript without requiring credentials; the session layer also covers first AI SDK-emitted tool-call dispatch in smoke fixtures.
...
- The CLI still intentionally defaults to the deterministic fake-provider path until full live session orchestration, cancellation, tool schema advertisement/continuation, and retry scheduling are wired.
