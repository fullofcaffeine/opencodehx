...
- The CLI still intentionally defaults to the deterministic fake-provider path when no local config model or default-agent model is present, until config-backed live provider selection moves behind the server, abort signals propagate through that server-owned live loop, remaining upstream prompt/session lifecycle edges, and real retry backoff timing/cancellation lifecycle are wired.
