...
- `run --session <id>` now validates a stored session through the configured SQLite database, reuses the recovered session ID, and uses the stored session directory when `--dir` is absent. This is resume wiring evidence, not full history-aware continuation.
