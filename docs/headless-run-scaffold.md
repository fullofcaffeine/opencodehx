...
- The CLI still intentionally defaults to the deterministic fake-provider path when no local config model or default-agent model is present, until remaining upstream prompt/session lifecycle edges and broader retry backoff plus user-facing cancellation lifecycle wiring are covered.
