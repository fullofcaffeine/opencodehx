...
- The CLI still intentionally defaults to the deterministic fake-provider path when no local config model or default-agent model is present, until full live session orchestration, server-backed cancellation routing, remaining upstream prompt/session lifecycle edges, and retry loop scheduling are wired.
