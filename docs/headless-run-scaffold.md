...
- `run --live-ai-sdk --model provider/model` is an opt-in live path through `ProviderRegistry.getLanguage` and the real AI SDK stream facade. It loads well-known remote configs from `wellknown` auth entries, the XDG global config directory, project config for the run directory, `OPENCODE_AUTH_CONTENT`, and the upstream-shaped XDG data `auth.json` file before resolving providers.
