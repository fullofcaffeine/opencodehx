...
- Live AI SDK requests now use the existing `SessionLlm` assembly path for both initial calls and tool-result continuations. The stream facade forwards max output tokens, temperature, `topP`, `topK`, request headers, transformed provider options, `maxRetries: 0`, and `--variant`-selected model options into `streamText(...)`; `SessionProcessorSmoke` verifies those fields through `MockLanguageModelV3.doStreamCalls`. Selected/default agent config now feeds that same live path for agent name, prompt-as-system, model fallback, variant fallback, provider options, temperature, `top_p`, disabled tool filtering, and agent permission rules.
- The CLI still intentionally defaults to the deterministic fake-provider path when no local config model or default-agent model is present, until full live session orchestration, cancellation, remaining upstream message-history edges, and retry scheduling are wired.
