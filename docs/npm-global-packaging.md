...
**Beads:** `opencodehx-037`, `opencodehx-61d`, `opencodehx-5kz`, `opencodehx-d5b`, `opencodehx-jr4`, `opencodehx-x4i`, `opencodehx-o48`
...
- the installed bin passes `--version`, `--help`, deterministic `run --model openai/gpt-5.2`, deterministic `run --dir <workspace> --format json`, deterministic `run --file <path>` attachment ingestion, mock AI SDK `run --mock-ai-sdk --dir <workspace> --format json`, deterministic and mock AI SDK `OPENCODE_DB`-backed `run`/`export`/`run --session` append, deterministic persisted file-part export, deterministic `run --continue` append, and `serve --help`;
...
Installed `run --dir`, `run --file`, and `OPENCODE_DB` evidence is still a headless bootstrap smoke, not full OpenCode chat parity. It proves the package can resolve a real workspace path, record local file attachment metadata as user parts, run both deterministic and mock AI SDK session paths from the installed binary, persist fresh sessions, export them, append by explicit session ID, append deterministic runs through latest-root `--continue`, and use recovered text history in the resumed AI SDK prompt. It does not yet construct full tool/file-aware history prompts for prior sessions, initialize projects, attach to a server, prompt for permissions, perform richer live provider attachment handling, or perform live provider chat without the explicit `--live-ai-sdk` opt-in path.
