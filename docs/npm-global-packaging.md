...
- installed `run --live-ai-sdk` evidence asserts the local OpenAI-compatible request path, bearer authorization header, `stream: true` request body, OPENCODE_DB-backed export, and resumed append while avoiding external credentials;
