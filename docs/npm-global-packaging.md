...
- the installed bin passes `--version`, `--help`, deterministic `run --model openai/gpt-5.2`, deterministic `run --dir <workspace> --format json`, mock AI SDK `run --mock-ai-sdk --dir <workspace> --format json`, deterministic and mock AI SDK `OPENCODE_DB`-backed `run`/`export`/`run --session` append, deterministic `run --continue` append, and `serve --help`;
