...
| PTY | `pty/index.ts`, `pty/pty.node.ts`, `pty/pty.bun.ts`, `server/routes/instance/pty.ts` | `node-host`/`bun-host` | `PtyService` owns active sessions and lifecycle events over `@lydell/node-pty`; `PtySmoke` covers create/exit/delete and login-shell args. | Bun implementation uses `bun-pty`. WebSocket control frames, output buffering, and subscriber isolation are next. |
