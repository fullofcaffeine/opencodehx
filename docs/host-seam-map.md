...
| File watching | `file/watcher.ts`, `project/vcs.ts` | `node-host` facade with typed app events | `FileWatcherRuntime` wraps Node `fs.watch` into `FileUpdatedEvent` values and filters git metadata watching to `.git/HEAD` for VCS branch refresh. `ProjectRuntimeSmoke` uses an injected backend and `npm run file:watcher:smoke` exercises the real native backend outside normal CI. | Upstream uses `@parcel/watcher` with backend-specific bindings and config/protected ignore integration; broader root watching and backend selection remain later file service work. |
...
| Server HTTP/WebSocket adapter | `server/adapter.node.ts`, `server/adapter.bun.ts`, `server/server.ts` | `node-host`/`bun-host` | `NodeHonoAdapter` plus narrow Hono/node-ws/node-server externs cover listener start/stop, WebSocket injection, and first route smoke evidence. | Both adapters should implement one Haxe `ServerAdapter` interface with `listen`, `stop`, and `upgradeWebSocket`. Preserve port-0 fallback to 4096 then random. |
...
| Process spawning and shell | `shell/shell.ts`, `tool/bash.ts`, `lsp/server.ts`, `util/process.ts` | `node-host` | `NodeProcess` facade for shell selection, `spawnSync`, cwd/env, stdout/stderr capture, exit code, timeout metadata, and upstream-style `killTree`. | Deterministic fixtures cover preferred/acceptable shell selection, Git Bash fallback, PowerShell fallback, environment case sensitivity, and POSIX kill-tree teardown; `windows-shell-parity` CI covers native Windows shell execution and `taskkill` teardown. |
| PTY | `pty/index.ts`, `pty/pty.node.ts`, `pty/pty.bun.ts`, `server/routes/instance/pty.ts` | `node-host`/`bun-host` | `PtyService` owns active sessions, lifecycle events, output buffering, cursor control frames, write/resize, and subscriber isolation over `@lydell/node-pty`; `PtySmoke` and `ServerSmoke` cover service and Hono WebSocket behavior. | Bun implementation uses `bun-pty`; shell-selection and platform-specific PTY differences remain follow-up work. |
...
| Clock and timers | `id/id.ts`, `session/session.ts`, `mcp/auth.ts`, TUI contexts | portable facade | `opencodehx.host.Clock.nowMillis` and `Clock.parseHttpDateMillis` now wrap typed JS `Date` externs for project and retry timing; add `Clock.sleep`/`Clock.timeout` when timer users land. | Required for deterministic ids, sessions, auth expiry, debounce tests, and TUI replays. |
...
| Resources/assets | prompt `.txt`, theme `.json`, sound `.wav`, tree-sitter `.wasm` imports | `resource` | `opencodehx.resource.Resources` resolves copied text/file/WASM assets under Node; JSON import attributes use `genes.ts.Imports.defaultImportWith`. `genes.ts.Imports.text`, `file`, `dynamicWith`, and `dynamicWasm` are available for hosts with an explicit loader/bundler contract. | Do not inline large resources accidentally. Keep the copied-resource adapter for plain Node paths unless direct imports have runtime evidence. |
