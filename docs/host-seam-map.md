...
| Process spawning and shell | `shell/shell.ts`, `tool/bash.ts`, `lsp/server.ts`, `util/process.ts` | `node-host` | `NodeProcess` facade for shell selection, `spawnSync`, cwd/env, stdout/stderr capture, exit code, and timeout metadata. | Deterministic fixtures cover preferred/acceptable shell selection, Git Bash fallback, PowerShell fallback, and environment case sensitivity; native Windows process execution still needs platform CI/manual smoke. |
