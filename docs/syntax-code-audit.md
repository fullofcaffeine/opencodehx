...
| `src/opencodehx/provider/AiSdkProvider.hx` | Justified AI SDK prompt middleware boundary | `wrapLanguageModel` receives an AI SDK `LanguageModelV3Prompt`, while `ProviderTransform.message` returns OpenCodeHX's wider provider prompt record used by transform fixtures. The raw snippet is a single TypeScript assertion back to the SDK prompt union after the transform; the runtime shape is unchanged and all app-facing code stays on typed prompt DTOs. |
