...
- `opencodehx.plugin.PluginMeta` tracks file and npm plugin metadata in JSON: source, requested version, installed version, file modified time, fingerprint, load count, and first/same/updated states. Package metadata and persisted metadata are decoded through `genes.ts.UnknownRecord` before app code reads fields.
