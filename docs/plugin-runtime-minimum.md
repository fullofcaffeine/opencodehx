...
OpenCodeHX now has a dependency-free first plugin runtime seam. The port matrix now promotes exact evidence for `plugin/shared.test.ts`, `plugin/meta.test.ts`, `plugin/loader-shared.test.ts`, `plugin/trigger.test.ts`, the pure JWT/account-id helper part of `plugin/codex.test.ts`, the Cloudflare AI Gateway chat-params rule from `plugin/cloudflare.test.ts`, and the GitHub Copilot plugin model merge/remap rules from `plugin/github-copilot-models.test.ts`; install/auth/built-in provider/workspace plugin rows remain deferred.
