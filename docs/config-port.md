...
- Managed config source parsing for MDM/mobileconfig JSON, including `JsonCodec`-validated metadata-key stripping and final managed-over-user precedence through an explicit loader source.
...
- LSP config refinement for the upstream boundary rule: boolean toggles pass, builtin server IDs such as `typescript` can omit `extensions`, disabled custom servers can omit `extensions`, and enabled custom servers must declare a string `extensions` array. The refinement now narrows through `genes.ts.UnknownNarrow` while raw `ConfigInfo.lsp` storage stays deferred to the live LSP schema slice.
...
- Environment reads for config loading and `{env:...}` substitution now go through the `NodeProcess` host seam and the `ConfigEnv` wrapper. Provided env maps narrow property reads through `genes.ts.UnknownNarrow`; the remaining mutable write boundary is limited to string token injection for remote config auth.
...
- TUI config normalization covers nested `tui` flattening, top-level override precedence, keybind merges, Windows terminal suspend/input undo normalization, plugin spec merge/dedup by package identity, tuple option preservation, plugin provenance, and `plugin_enabled` merges.
- Legacy TUI migration moves `theme`, `keybinds`, and nested `tui` values out of `opencode.{json,jsonc}` into `tui.json`, writes a backup, strips migrated keys from the main config, drops unknown nested TUI keys, and still loads generated TUI config if the legacy source cannot be stripped.
...
Markdown frontmatter is intentionally typed as an `unknown` boundary at parse time. Command and agent loaders immediately narrow the fields they own into typed Haxe records; `ConfigCommand` reads command string/boolean fields and `ConfigAgent` reads agent fields, permission records, and tool maps through `genes.ts.UnknownNarrow` rather than reflection or casts. Unknown agent frontmatter keys survive only through the documented `options` passthrough.
