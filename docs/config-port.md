...
Markdown frontmatter is intentionally typed as an `unknown` boundary at parse time. Command and agent loaders immediately narrow the fields they own into typed Haxe records; `ConfigAgent` reads agent fields, permission records, and tool maps through `genes.ts.UnknownNarrow` rather than reflection. Unknown agent frontmatter keys survive only through the documented `options` passthrough.
