...
`opencodehx.effect.RuntimeMemo` and `RunServiceRuntime` cover the stable memo-map behavior from upstream `makeRuntime`: separately-created runtimes can depend on the same shared layer and see one initialized dependency. `EffectSmoke.runServiceMemoMap()` creates two runtime services over one memoized shared service, proves both return the same shared ID, and proves the dependency factory ran once. `EffectSmoke.runServiceAsync()` covers Promise-backed service execution, typed success/failure exits, callback success/failure delivery, fork success/failure/interruption, and proves async calls reuse the initialized service.
