...
This does not port the full OpenCode `File.Service`, cache/state machine, fuzzysort search, full watcher/fsmonitor integration, protected file rules, file patch/diff read payloads, ripgrep download/bootstrap, ripgrep worker-mode split, or Effect streaming. `file/path-traversal.test.ts` also includes `Instance.containsPath` worktree/monorepo cases; those remain owned by the project runtime seam rather than the file primitive. The first tool slices should use these primitives directly, then promote repeated needs into a richer service facade.
