...
This does not port the full OpenCode `File.Service`, cache/state machine, fuzzysort search, watcher/fsmonitor integration, protected file rules, file patch/diff reads, ripgrep download/bootstrap, or Effect streaming. The first tool slices should use these primitives directly, then promote repeated needs into a richer service facade.
