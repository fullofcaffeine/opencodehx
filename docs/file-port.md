...
- `opencodehx.file.AppFileSystem` covers the upstream shared filesystem helper surface used by the app: directory/file predicates, size checks, text/byte/JSON reads and writes, formatted JSON, file modes, Web and Node stream writes, recursive writes, ancestor search with root-first ordering, simple glob helpers, exists/remove passthrough, MIME lookup, Windows path conversion, resolve fallback/canonicalization, symlink resolve errors where the host supports them, containment, and overlap checks.
