...
- Local no-network OpenAI-compatible fixtures prove streaming success, provider-error transcript/export, live `read`/`write`/`edit`/`apply_patch`/`bash` tool calls, write-then-read tool chains, config-denied write calls, explicit-deny precedence under `--dangerously-skip-permissions`, and skip-flag approval for config-asked writes.
...
- `account-repo.md` covers the SQLite-backed account repository and credential-free service seams: account row/state persistence, active org selection, token updates, URL normalization, token refresh/coalescing, config/org headers, and device-poll state mapping.
- `config-port.md`, `permission-model-port.md`, `tool-registry-port.md`, `bash-shell-seam.md`, `patch-runtime.md`, `snapshot-runtime.md`, `share-next-runtime.md`, `project-runtime-parity.md`, `formatter-port.md`, and `effect-runtime-parity.md` cover config loading including account-backed remote config/token substitution and TUI config migration/precedence, permissions with async pending-lifecycle and server route evidence, typed tool registry/custom `.opencode/tool(s)` discovery, parser-backed bash permissions, the standalone patch namespace, focused snapshot track/patch/revert/diff behavior, ShareNext request routing, create/remove persistence, and latest-diff sync coalescing, project/git/sync/workspace-restore foundations, formatting, observability resources, AppRuntime logger/context evidence, cross-spawn process-handle evidence, run-service memo behavior, instance state, and Runner shared-run/cancel behavior.
- `storage-port.md`, `resource-imports.md`, `bus-runtime-parity.md`, `pty-runtime.md`, and `fixture-smoke-parity.md` cover storage persistence, serialized JSON updates, JSON migration, host/runtime resources, event delivery, PTY lifecycle/WebSocket replay, and smoke tmpdir parity.
...
- `mcp-acp-minimum.md`, `lsp-minimum.md`, and `plugin-runtime-minimum.md` cover the first MCP/ACP, LSP, and plugin runtime surfaces, including plugin metadata serialization and async plugin trigger awaiting.
