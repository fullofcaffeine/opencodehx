...
**Beads:** `opencodehx-017`, `opencodehx-nmox`, `opencodehx-6pcb`, `opencodehx-s0vk`
...
- `edit` for exact and multiline replacement, tolerant upstream-style fallback matching, `replaceAll`, empty-`oldString` file creation, CRLF/line-ending and UTF-8 BOM preservation including formatter-strip restore, identical-input and directory-path failures, duplicate-match rejection, diff metadata, and external-directory permission requests.
- `apply_patch` for OpenAI-style patch envelopes with add, update, delete, move planning, insert-only hunks, UTF-8 BOM preservation including formatter-strip restore, EOF/context anchors, heredoc-wrapped patches, whitespace/Unicode-normalized matching, external-directory permission requests, permission aggregation, and summary output.
