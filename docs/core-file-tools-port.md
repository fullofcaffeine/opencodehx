...
- `read` for file previews, line offsets/limits, long-line truncation suffixes, directory listings with pagination, image/PDF data-url attachments with content sniffing, nearby instruction reminders with per-message claim dedupe, out-of-range offset errors, external-directory permission requests, missing-file suggestions, and binary-file rejection.
