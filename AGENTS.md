...
For untrusted JSON/JS values, treat local unknown-narrowing helpers as quarantine, not architecture. OpenCodeHX owns domain decoders and error policy, but reusable primitives such as `UnknownNarrow`, `UnknownRecord`, and read-only unknown arrays belong in `../genes` as generic `genes.ts` helpers with clean TS lowering. Until those exist, any local helper must combine guard + conversion, avoid public `isX/asX` pairs, prove own-property access before reads, distinguish `null` from `undefined` when the boundary cares, and document that it is temporary compiler/library debt.
