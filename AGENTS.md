...
- Prefer module-level functions when the API is naturally stateless and the surrounding module already provides the namespace. Do not create empty "shell" classes solely to host public static functions unless target interop, macros, extern shape, codegen constraints, or an established local pattern genuinely requires a class.
...
- **Hard rule: type quality must meet or beat upstream TypeScript.** OpenCodeHX source should be at least as typed as careful handwritten TypeScript for the same behavior, and preferably stronger where Haxe can recover invariants. Do not accept weaker Haxe models merely because upstream used broad JavaScript/TypeScript shapes. If upstream has an `any`, `unknown`, stringly key, open record, or assertion, first ask whether Haxe can model the real domain with an enum, abstract, typedef, narrow extern, decoder, or macro-checked constructor while preserving runtime parity.
- **Hard rule: typed domain values by default.** Prefer precise Haxe domain types, enums/enum abstracts, abstracts/newtypes, typed records, narrow externs/facades, and validated decoders over stringified keys, stringly open records, `DynamicAccess`, `Dynamic`, `untyped`, `cast`, generated `any`, broad `unknown`, or raw target-language snippets. These weak forms are allowed only at true runtime/interop/compiler/test boundaries after a practical typed alternative has been ruled out. Every exception must stay as narrow and local as possible, include a nearby boundary comment explaining why it exists, what contains the unsafety, and what owner/evidence can remove or narrow it later, and must not leak into app-facing APIs.
...
- **Hard rule: avoid reflection in product code.** Treat `Reflect.field`, `Reflect.setField`, `Reflect.hasField`, `Reflect.fields`, `Reflect.deleteField`, `Reflect.isObject`, and similar reflection APIs as typed-boundary debt. Prefer typedefs, enums, abstracts, `DynamicAccess<T>` only for real string-keyed maps, `genes.ts.Unknown` narrowing, typed JSON codecs, or small named facades. Reflection is acceptable only inside a documented decoder/interop/compiler boundary where the field set is genuinely not known to Haxe yet; copy validated values into typed records before they reach app logic.
- Run `npm run typed-boundary:scan` when changing Haxe source that touches JSON, tool calls, provider/config data, storage rows, externs, or generated TypeScript hygiene. The scan is a ratchet: reducing weak markers is fine, but new or increased `Dynamic`, `DynamicAccess`, `cast`, `untyped`, raw `Syntax.code`, `@:ts.type`, or reflection markers require a deliberate boundary justification and baseline update.
...
Current checked artifact constructors are documented in `docs/checked-artifact-constructors.md`. In addition to TUI routes/keybinds and tool IDs, built-in provider IDs should use `ProviderIDs.known("...")` or `KnownProviderID`, TUI dialog fixture provider IDs should use `TuiProviderIDs.known("...")`, source-authored server/SSE events should use `ServerEventTypes.known("...")`, and Haxe-owned copied-resource reads should use `ResourcePaths.known("...")`. Fixed JS harness package/generated/resource paths should use `scripts/harness/paths.mjs` where practical. Do not force config keys, plugin-defined provider IDs, `models.dev` model IDs, typo-suggestion fixtures, or runtime-discovered harness paths into Haxe macros; keep them as boundary data.
