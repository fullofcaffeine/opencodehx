...
export class TuiScaffold {
	...
	static renderView(): JSX.Element {
		...
		var text: JSX.Element = <text fg={theme.primary}>OpenCodeHX TUI</text>;
		var text_1: JSX.Element = <text>{"Route: " + routeLabel}</text>;
		var text_2: JSX.Element = <text>{"Theme: " + TuiScaffold.foundation.theme.selected() + " " + TuiScaffold.foundation.theme.mode()}</text>;
		var text_3: JSX.Element = <text>{"Key: " + key}</text>;
		var text_4: JSX.Element = <text>{user.label + ": " + user.text}</text>;
		var text_5: JSX.Element = <text>{tool.label + ": " + tool.text}</text>;
		var text_6: JSX.Element = <text>{assistant.label + ": " + assistant.text}</text>;
		var text_7: JSX.Element = <text>{meta.label + ": " + meta.text}</text>;
		var text_8: JSX.Element = <text>{modelTitle.label + ": " + modelTitle.text}</text>;
		var text_9: JSX.Element = <text>{modelOption.label + ": " + modelOption.text}</text>;
		var text_10: JSX.Element = <text>{providerTitle.label + ": " + providerTitle.text}</text>;
		var text_11: JSX.Element = <text>{providerOption.label + ": " + providerOption.text}</text>;
		var text_12: JSX.Element = <text>{sessionTitle.label + ": " + sessionTitle.text}</text>;
		var text_13: JSX.Element = <text>{sessionOption.label + ": " + sessionOption.text}</text>;
		var text_14: JSX.Element = <text>{permissionTitle.label + ": " + permissionTitle.text}</text>;
		var text_15: JSX.Element = <text>{permissionRequest.label + ": " + permissionRequest.text}</text>;
		var text_16: JSX.Element = <text>{permissionChoices.label + ": " + permissionChoices.text}</text>;
		var text_17: JSX.Element = <text>{modelAction.label + ": " + modelAction.text}</text>;
		var text_18: JSX.Element = <text>{providerAction.label + ": " + providerAction.text}</text>;
		var text_19: JSX.Element = <text>{sessionAction.label + ": " + sessionAction.text}</text>;
		var text_20: JSX.Element = <text>{permissionAllowAction.label + ": " + permissionAllowAction.text}</text>;
		var text_21: JSX.Element = <text>{permissionRejectAction.label + ": " + permissionRejectAction.text}</text>;
		var input: JSX.Element = <input focused value="" />;
		return <box flexDirection="column">{text}{text_1}{text_2}{text_3}{text_4}{text_5}{text_6}{text_7}{text_8}{text_9}{text_10}{text_11}{text_12}{text_13}{text_14}{text_15}{text_16}{text_17}{text_18}{text_19}{text_20}{text_21}{input}</box>;
	}
