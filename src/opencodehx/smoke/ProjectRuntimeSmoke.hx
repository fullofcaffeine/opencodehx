...
class ProjectRuntimeSmoke {
	...
	static function npmRuntime(root:String):Void {
		...
		eq(fixture.requests[fixture.requests.length - 1].add.join(","), "prettier", "npm install empty version add spec");
		...
		final requestedCleanDir = directory(root, "npm-install-requested-clean");
		Fs.mkdirSync(NodePath.join(requestedCleanDir, "node_modules"), {recursive: true});
		write(requestedCleanDir, "package.json", "{}");
		write(requestedCleanDir, "package-lock.json", '{"packages":{"":{"dependencies":{"eslint":"9.0.0"}}}}');
		final beforeRequestedClean = fixture.requests.length;
		...
		final optionalCleanDir = directory(root, "npm-install-optional-clean");
		Fs.mkdirSync(NodePath.join(optionalCleanDir, "node_modules"), {recursive: true});
		write(optionalCleanDir, "package.json", '{"optionalDependencies":{"fsevents":"2.0.0"}}');
		write(optionalCleanDir, "package-lock.json", '{"packages":{"":{"optionalDependencies":{"fsevents":"2.0.0"}}}}');
		final beforeOptionalClean = fixture.requests.length;
		NpmRuntime.install(fixture.deps, optionalCleanDir, {add: []});
		eq(fixture.requests.length, beforeOptionalClean, "npm install optional dependency lock skips reify");
		...
		final objectSingleDir = NpmRuntime.cacheDirectory(fixture.deps, "@scope/object-single");
		final objectSingleBin = NodePath.join(NodePath.join(objectSingleDir, "node_modules"), ".bin");
		final objectSinglePkg = NodePath.join(NodePath.join(objectSingleDir, "node_modules"), NodePath.join("@scope", "object-single"));
		Fs.mkdirSync(objectSingleBin, {recursive: true});
		Fs.mkdirSync(objectSinglePkg, {recursive: true});
		writeFile(NodePath.join(objectSingleBin, "fallback"), "#!/bin/sh\n");
		writeFile(NodePath.join(objectSingleBin, "tool"), "#!/bin/sh\n");
		...
		eq(NpmRuntime.outdated(fixture.deps, "prettier", "<=2.9.0"), false, "npm outdated less-than-or-equal range satisfied");
	...
	static function installationRuntime():Void {
		...
		final brewPullFailure = installationFixture("/usr/local/bin/opencode");
		brewPullFailure.outputs.set("brew list --formula anomalyco/tap/opencode", processOk("opencode\n"));
		brewPullFailure.outputs.set("brew --repo anomalyco/tap", processOk("/tmp/homebrew-tap\n"));
		brewPullFailure.outputs.set("git pull --ff-only", {code: 3, stdout: "", stderr: "pull failed"});
		final brewPullFailureResult = InstallationRuntime.upgrade(brewPullFailure.deps, InstallationMethod.Brew, "9.9.9");
		eq(brewPullFailureResult.code, 3, "installation brew pull failure code");
		eq(brewPullFailureResult.stderr, "pull failed", "installation brew pull failure stderr");
