  ...
  const exact = new Map([
    ...
    [
      ...
      [
        ...
        "focused snapshot runtime covers deterministic/repeated track hashes, project state isolation, secondary worktree patch/revert/diff isolation, patch detection for added/modified/deleted files including special ASCII filenames, Unicode filenames and nested Unicode paths, long filenames, hidden files, and file/nested-directory/circular symlink patch evidence on capable hosts, under-limit added-file tracking, chmod-only no-op changes, revert restore/delete behavior including recreated-file, nested added files, empty parent directories after added-file removal, non-existent requested files, overlapping first-patch, same-hash file/directory patch order, large mixed batch, and added/modified binary cases, focused restore of snapshot files while preserving new files, empty-directory no-op, invalid-hash empty patch, large added-file skip/stable hash, gitignore filtering including files newly ignored after a snapshot, .git/info/exclude rules, and global core.excludesFile rules, simple diff/diffFull evidence including no-change, deterministic order, added/modified/deleted and multiline-added text patch metadata, mixed and multiple add/delete entries, large interleaved text/binary mixed diffs, whitespace edits, added/deleted/modified statuses, grow-only/trim-only text churn counts, and binary diffFull empty-patch/zero-churn shape; persistent separate Git-dir lifecycle, full restore edge cases, richer text diffFull patch metadata, symlink restore/lifecycle edge cases, cleanup/prune, and concurrency behavior remain deferred",
        "SnapshotSmoke",
        m11Owners.foundation,
      ],
    ],
