...
function directEvidence(path) {
  const exact = new Map([
    ...
    ["config/config.test.ts", ["partial", "src/opencodehx/smoke/ConfigSmoke.hx; src/opencodehx/smoke/ServerSmoke.hx; docs/config-port.md", "real account repo/service integration, runtime plugin loading/path resolution, and live package-manager dependency install side effects remain deferred; deterministic dependency bootstrap gitignore/install success/failure is covered", "ConfigSmoke plus ServerSmoke GET /config and GET /config/providers route fixtures"]],
    [
      "config/tui.test.ts",
      [
        "partial",
    ...
    ["plugin/auth-override.test.ts", ["partial", "src/opencodehx/plugin/PluginAuthHooks.hx; src/opencodehx/plugin/PluginConfigHooks.hx; src/opencodehx/smoke/PluginSmoke.hx; src/opencodehx/smoke/ProviderSmoke.hx; src/opencodehx/smoke/ServerSmoke.hx; docs/plugin-runtime-minimum.md; docs/provider-registry-port.md", "typed auth-method override precedence, prompt metadata modeling, config-hook error isolation, and first server provider-auth method output are covered; live plugin file loading, ProviderAuth authorize/callback service integration, auth loader/callback persistence, and install side effects remain deferred", "PluginSmoke.authOverride plus ProviderSmoke registryPluginConfigHooks isolation plus ServerSmoke GET /provider/auth fixture", m11Owners.plugin]],
    ...
    ["provider/provider.test.ts", ["partial", "src/opencodehx/smoke/ProviderSmoke.hx; src/opencodehx/smoke/AiSdkProviderSmoke.hx; src/opencodehx/smoke/ProviderTransformSmoke.hx; src/opencodehx/smoke/ServerSmoke.hx; docs/provider-registry-port.md", "broader live provider loading, external credential chains, OAuth authorize/callback routes, and full upstream provider service lifecycle remain deferred", "ProviderSmoke registry/env/config/auth/filter/model fixtures plus AiSdkProviderSmoke factory paths, ProviderTransformSmoke request/message transforms, SessionProcessorSmoke live provider-transform middleware evidence, and ServerSmoke GET /config/providers, GET /provider, plus GET /provider/auth fixtures"]],
