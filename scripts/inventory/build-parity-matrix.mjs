...
function directEvidence(path) {
  const exact = new Map([
    ...
    ["tool/read.test.ts", ["partial", "src/opencodehx/smoke/ToolSmoke.hx; docs/core-file-tools-port.md", "representative read file/directory/error behavior, file offset errors, long-line truncation suffixes, directory pagination, image/PDF media attachments with content sniffing, absolute read permission patterns, external-directory read prompts, nearby instruction reminder metadata, and per-message instruction claim dedupe/clear are covered; fuller upstream output matrix remains deferred", "ToolSmoke.readExec plus PermissionSmoke.toolIntegration"]],
