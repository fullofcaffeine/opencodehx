...
function directEvidence(path) {
  const exact = new Map([
    ...
    ["tool/read.test.ts", ["partial", "src/opencodehx/smoke/ToolSmoke.hx; docs/core-file-tools-port.md", "representative read file/directory/error behavior, file offset errors, byte-cap and line-count truncation footers, upstream-shaped missing-file suggestions, long-line truncation suffixes, known binary extension rejection, directory pagination, symlinked-directory suffixes, image/PDF media attachments with content sniffing, absolute read permission patterns, external-directory read prompts, nearby instruction reminder metadata, and per-message instruction claim dedupe/clear are covered; fuller upstream output matrix remains deferred", "ToolSmoke.readExec plus PermissionSmoke.toolIntegration"]],
    ["tool/write.test.ts", ["partial", "src/opencodehx/smoke/ToolSmoke.hx; docs/core-file-tools-port.md", "write creation, overwrite metadata, JSON/binary-safe/empty/multiline/CRLF content preservation, relative title output, diff/filediff metadata, BOM preservation, and external-directory write permission shape are covered; LSP diagnostics/formatter/bus integration matrix remains deferred", "ToolSmoke.writeExec"]],
    [
      "tool/edit.test.ts",
      [
        "partial",
