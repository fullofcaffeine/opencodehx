...
function directEvidence(path) {
  const exact = new Map([
    ...
    ["bus/bus-effect.test.ts", ["partial", "src/opencodehx/bus/BusStreamRuntime.hx; src/opencodehx/bus/GlobalBusRuntime.hx; src/opencodehx/smoke/BusSmoke.hx; docs/bus-runtime-parity.md", "observable stream delivery, stream unsubscribe, subscribeAll filtering, multi-subscriber behavior, upstream-shaped GlobalBus event emission, and server.instance.disposed directory payload are covered; full upstream Effect Layer/Deferred/scoped fiber and Instance.provide service wiring remain deferred", "BusSmoke streamDelivery/streamUnsubscribe/globalBusEmit/instanceDisposal"]],
