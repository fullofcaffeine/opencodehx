  ...
  const exact = new Map([
    ...
    [
      ...
      [
        ...
        "focused snapshot runtime covers deterministic/repeated track hashes, patch detection for added/modified/deleted files including symlink patch evidence on capable hosts, revert restore/delete behavior including recreated-file, overlapping first-patch, and added/modified binary cases, focused restore of snapshot files while preserving new files, empty-directory no-op, invalid-hash empty patch, large added-file skip/stable hash, gitignore filtering, simple diff/diffFull evidence including added/deleted/modified statuses, and binary diffFull empty-patch/zero-churn shape; persistent separate Git-dir lifecycle, full restore edge cases, rich text diffFull patch metadata, worktree isolation, symlink restore/lifecycle edge cases, cleanup/prune, and concurrency behavior remain deferred",
        "SnapshotSmoke",
        m11Owners.foundation,
      ],
    ],
