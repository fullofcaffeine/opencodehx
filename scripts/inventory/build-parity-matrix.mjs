...
function directEvidence(path) {
  const exact = new Map([
    ...
    ["account/service.test.ts", ["partial", "src/opencodehx/account/AccountService.hx; src/opencodehx/account/AccountRepo.hx; src/opencodehx/smoke/AccountSmoke.hx; docs/account-repo.md", "login normalization, transport errors, org grouping, config headers, poll success persistence, and poll status mapping are covered through an injected HTTP client; eager token refresh, refresh persistence, concurrent refresh coalescing, and full Effect service layering remain deferred", "AccountSmoke async account service fixtures", m11Owners.provider]],
