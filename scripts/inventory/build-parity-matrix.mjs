...
function directEvidence(path) {
  const exact = new Map([
    ...
    ["installation/installation.test.ts", ["partial", "src/opencodehx/smoke/ProjectRuntimeSmoke.hx; docs/project-runtime-parity.md", "installation method detection for npm/yarn/pnpm/bun/brew/scoop/choco/POSIX-and-Windows-curl/unknown, latest-version lookup for unknown-method GitHub fallback plus GitHub/npm/pnpm/bun/brew/scoop/choco shapes, release-type classification including v-prefixed tags, curl install-script upgrade planning, npm/pnpm/Bun/Scoop/Chocolatey upgrade command planning with success version probes, Homebrew core/tap upgrade planning plus empty-repo skip and tap/pull failure containment, Chocolatey elevated-shell failure messaging, unknown-method upgrade failure, and package-manager uninstall command planning plus no-op methods are covered with typed injected HTTP/process seams; real package-manager side effects remain deferred", "ProjectRuntimeSmoke installationRuntime fixture", m11Owners.foundation]],
