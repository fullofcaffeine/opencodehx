...
function directEvidence(path) {
  const exact = new Map([
    ...
    ["server/global-session-list.test.ts", ["partial", "src/opencodehx/smoke/ServerSmoke.hx; docs/server-hono-seam.md", "route-level smoke covers /experimental/session listing, project metadata, cursor pagination, search, and archived-session exclusion/inclusion; true multi-project service-backed listing remains deferred", "ServerSmoke GET /experimental/session fixtures plus PATCH /session/:id archive fixture", m11Owners.server]],
