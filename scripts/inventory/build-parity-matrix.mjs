...
function directEvidence(path) {
  const exact = new Map([
    ...
    ["tool/read.test.ts", ["partial", "src/opencodehx/smoke/ToolSmoke.hx; docs/core-file-tools-port.md", "representative read file/directory/error behavior, file offset errors, directory pagination, image/PDF media attachments with content sniffing, absolute read permission patterns, external-directory read prompts, nearby instruction reminder metadata, and per-message instruction claim dedupe/clear are covered; fuller upstream output matrix remains deferred", "ToolSmoke.readExec plus PermissionSmoke.toolIntegration"]],
    ...
    [
      ...
      [
        ...
        "route-level smoke covers upstream abort action success response, missing-session validation, status event routing, injected live AI SDK active status observation, and active live abort propagation",
        "ServerSmoke POST /session/:id/abort, /session/status, and injected live AI SDK /session route",
        "none",
      ],
    ],
