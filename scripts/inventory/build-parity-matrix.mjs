...
function directEvidence(path) {
  const exact = new Map([
    ...
    ["config/config.test.ts", ["partial", "src/opencodehx/smoke/ConfigSmoke.hx; docs/config-port.md", "remote/account config, remote skill URL downloads, runtime plugin loading, and dependency install side effects remain deferred", "ConfigSmoke"]],
