  ...
  const exact = new Map([
    ...
    [
      ...
      [
        ...
        "one-turn fake-provider transcript golden plus credential-free async AI SDK session/CLI harness including pure hasToolCalls helper coverage, prompt-permission active-tool filtering, tool-call repair/fallback, request-option merge order, chat parameter assembly, streamText option assembly, stream-prompt transform branching, telemetry option assembly, stream failure error mapping, workflow model state/preapproval shaping, workflow approval shaping, workflow tool-executor result/error shaping, active-tool-name and workflow preapproval filtering, LiteLLM/GitHub Copilot _noop compatibility tool injection, streaming request header assembly and override order, system prompt assembly/transform/request-message branching, public ModelMessage prompt input evidence for initial streamText calls, accumulated public assistant tool-call/tool-result continuation history, text/tool export ordering and tool input/output/error sanitization, AI SDK text/tool store-backed recovery plus stream error and abort propagation, first model-emitted tool-call dispatch, live tool schema advertisement, repeated continuation after successful tool results with explicit opt-out/max-continuation cap evidence, and live-registry CLI validation with well-known remote config plus global/project config/auth storage/active-account config discovery",
        m11Owners.session,
      ],
    ],
...
function portRecord(path) {
  ...
  const areaEvidence = {
    ...
    cli: "CliSmoke and cli-smoke.mjs cover the headless run scaffold, OPENCODE_DB-backed run persistence/export/resume append, non-interactive export <sessionID> JSON/sanitize behavior, first run --session recovery validation, and run --continue latest-root selection",
  }
