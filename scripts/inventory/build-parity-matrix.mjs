...
function directEvidence(path) {
  const exact = new Map([
    ...
    ["config/config.test.ts", ["partial", "src/opencodehx/smoke/ConfigSmoke.hx; docs/config-port.md", "real account repo/service integration, remote skill URL downloads, runtime plugin loading/path resolution, and dependency install side effects remain deferred", "ConfigSmoke"]],
    ["skill/skill.test.ts", ["partial", "src/opencodehx/smoke/SkillSmoke.hx; docs/skill-registry-port.md", "remote skill URL downloads, Effect service integration, and permission-filtered available(agent) remain deferred", "SkillRegistry local discovery smoke"]],
    ["session/system.test.ts", ["partial", "src/opencodehx/smoke/SkillSmoke.hx; docs/skill-registry-port.md", "full SystemPrompt service integration remains deferred", "SkillRegistry sorted verbose format smoke"]],
