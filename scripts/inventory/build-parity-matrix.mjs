...
function directEvidence(path) {
  const exact = new Map([
    ...
    ["project/worktree.test.ts", ["partial", "src/opencodehx/smoke/ProjectRuntimeSmoke.hx; docs/project-runtime-parity.md", "worktree slug/unique naming, branch, create, ready/failed events, project-ID sharing, sandbox tracking, typed upstream-order instance service graph, bootstrap/cache/disposal, command-executed initialization, start commands, and reset/clean are covered; concrete share/snapshot/live plugin/LSP service internals remain deferred", "ProjectRuntimeSmoke worktree and instance bootstrap graph fixtures", "opencodehx-obx"]],
