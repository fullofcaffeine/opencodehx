...
function directEvidence(path) {
  const exact = new Map([
    ...
    ["tool/external-directory.test.ts", ["partial", "src/opencodehx/smoke/ToolSmoke.hx; docs/bash-shell-seam.md; docs/core-file-tools-port.md", "external workdir denial is covered for bash, and canonical external-directory request/denial shape is covered for read file, glob directory, and grep file targets; broader mutating file-tool permission surface remains deferred", "ToolSmoke bashExec/readExec/globExec/grepExec external directory cases"]],
