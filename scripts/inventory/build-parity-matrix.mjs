...
function directEvidence(path) {
  const exact = new Map([
    ...
    ["bus/bus-effect.test.ts", ["partial", "src/opencodehx/bus/BusStreamRuntime.hx; src/opencodehx/smoke/BusSmoke.hx; docs/bus-runtime-parity.md", "observable stream delivery, subscribeAll filtering, multi-subscriber behavior, and upstream-shaped server.instance.disposed directory payload are covered; full upstream Effect Layer/Deferred/scoped fiber, GlobalBus.emit, and Instance.provide service wiring remain deferred", "BusSmoke streamDelivery/instanceDisposal"]],
