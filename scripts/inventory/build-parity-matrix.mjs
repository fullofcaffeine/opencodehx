...
function portRecord(path) {
  ...
  const areaEvidence = {
    ...
    cli: "CliSmoke and cli-smoke.mjs cover the headless run scaffold, run --file local file/directory attachment parts, OPENCODE_DB-backed deterministic/mock/live AI SDK run persistence/export/resume append/continue/fork, non-interactive export <sessionID> JSON/sanitize behavior, first run --session recovery validation, run --continue latest-root selection, and local no-network run --live-ai-sdk OpenAI-compatible streaming success",
  }
