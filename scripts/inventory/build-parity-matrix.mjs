  ...
  const exact = new Map([
    ...
    [
      ...
      [
        ...
        "one-turn fake-provider transcript golden plus credential-free async AI SDK session/CLI harness including pure hasToolCalls helper coverage, prompt-permission active-tool filtering, tool-call repair/fallback, request-option merge order, chat parameter assembly, workflow approval shaping, active-tool-name and workflow preapproval filtering, LiteLLM/GitHub Copilot _noop compatibility tool injection, streaming request header assembly and override order, system prompt assembly/transform/request-message branching, first model-emitted tool-call dispatch, live tool schema advertisement, repeated continuation after successful tool results, and live-registry CLI validation with well-known remote config plus global/project config/auth storage/active-account config discovery",
        m11Owners.session,
      ],
    ],
