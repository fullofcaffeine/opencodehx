...
function directEvidence(path) {
  const exact = new Map([
    ...
    ["provider/provider.test.ts", ["partial", "src/opencodehx/smoke/ProviderSmoke.hx; src/opencodehx/smoke/AiSdkProviderSmoke.hx; src/opencodehx/smoke/ProviderTransformSmoke.hx; src/opencodehx/smoke/ServerSmoke.hx; docs/provider-registry-port.md", "broader live provider loading, external credential chains, auth routes, and full upstream provider service lifecycle remain deferred", "ProviderSmoke registry/env/config/auth/filter/model fixtures plus AiSdkProviderSmoke factory paths, ProviderTransformSmoke request/message transforms, SessionProcessorSmoke live provider-transform middleware evidence, and ServerSmoke GET /config/providers plus GET /provider fixtures"]],
