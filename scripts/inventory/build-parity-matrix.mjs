...
function directEvidence(path) {
  const exact = new Map([
    ...
    ["tool/truncation.test.ts", ["partial", "src/opencodehx/tool/Truncate.hx; src/opencodehx/smoke/ToolSmoke.hx; docs/core-file-tools-port.md; docs/bash-shell-seam.md", "standalone Truncate output/write/cleanup covers default limits, head/tail direction, byte/line truncation, full-output spill files, retention cleanup, and Task-vs-Grep hints; read, bash, glob, and grep still expose representative inline truncation metadata/output, while full Effect layer wiring and every caller using the shared Truncate service remain deferred", "ToolSmoke.truncateRuntime plus read/bash/glob/grep truncation fixtures", m11Owners.tool]],
    [
