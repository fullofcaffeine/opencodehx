...
function directEvidence(path) {
  const exact = new Map([
    ...
    ["effect/runner.test.ts", ["partial", "src/opencodehx/effect/RunnerRuntime.hx; src/opencodehx/smoke/EffectSmoke.hx; docs/effect-runtime-parity.md", "real Effect Scope/Fiber interruption and scoped cleanup remain deferred", "EffectSmoke.runner"]],
    ["effect/run-service.test.ts", ["partial", "src/opencodehx/effect/RuntimeMemo.hx; src/opencodehx/effect/RunServiceRuntime.hx; src/opencodehx/smoke/EffectSmoke.hx; docs/effect-runtime-parity.md", "full Effect ManagedRuntime/Layer/Context.Service and async run APIs remain deferred", "EffectSmoke.runServiceMemoMap"]],
    ...
    ["plugin/auth-override.test.ts", ["partial", "src/opencodehx/plugin/PluginAuthHooks.hx; src/opencodehx/plugin/PluginConfigHooks.hx; src/opencodehx/smoke/PluginSmoke.hx; src/opencodehx/smoke/ProviderSmoke.hx; docs/plugin-runtime-minimum.md; docs/provider-registry-port.md", "typed auth-method override precedence and config-hook error isolation are covered; live plugin file loading, ProviderAuth service integration, auth loader/callback persistence, and install side effects remain deferred", "PluginSmoke.authOverride plus ProviderSmoke registryPluginConfigHooks isolation", m11Owners.plugin]],
    ...
    ["plugin/meta.test.ts", ["partial", "src/opencodehx/plugin/PluginMeta.hx; src/opencodehx/smoke/PluginSmoke.hx; docs/plugin-runtime-minimum.md", "file and npm metadata state transitions are covered; concurrent cross-process metadata serialization remains deferred", "PluginSmoke.metadata", m11Owners.plugin]],
    ...
    ["plugin/trigger.test.ts", ["partial", "src/opencodehx/plugin/PluginRuntime.hx; src/opencodehx/smoke/PluginSmoke.hx; docs/plugin-runtime-minimum.md", "synchronous system-transform hook ordering is covered; async hook awaiting and full Plugin service integration remain deferred", "PluginSmoke.runtime trigger hook order", m11Owners.plugin]],
    ...
    ["tool/glob.test.ts", ["partial", "src/opencodehx/smoke/ToolSmoke.hx; docs/tool-registry-port.md", "core glob cases plus typed permission request/denial shape are covered; full Effect/external-directory context parity remains deferred", "ToolSmoke.globExec"]],
    ["tool/grep.test.ts", ["partial", "src/opencodehx/smoke/ToolSmoke.hx; docs/tool-registry-port.md", "core grep cases, exact-file search, no-match output, and typed permission request/denial shape are covered; full external-directory/Effect context remains deferred", "ToolSmoke.grepExec"]],
