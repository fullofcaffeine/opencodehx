...
function directEvidence(path) {
  const exact = new Map([
    ...
    ["bus/bus-effect.test.ts", ["partial", "src/opencodehx/bus/BusStreamRuntime.hx; src/opencodehx/bus/GlobalBusRuntime.hx; src/opencodehx/smoke/BusSmoke.hx; docs/bus-runtime-parity.md", "observable stream delivery, typed and wildcard stream unsubscribe, subscribeAll filtering, multi-subscriber behavior, upstream-shaped GlobalBus event emission, server.instance.disposed directory payload, and post-disposal subscription/publish no-op behavior are covered; full upstream Effect Layer/Deferred/scoped fiber and Instance.provide service wiring remain deferred", "BusSmoke streamDelivery/streamUnsubscribe/streamSubscribeAllUnsubscribe/globalBusEmit/instanceDisposal/disposedSubscriptionNoop"]],
    ...
    ["effect/runner.test.ts", ["partial", "src/opencodehx/effect/RunnerRuntime.hx; src/opencodehx/smoke/EffectSmoke.hx; docs/effect-runtime-parity.md", "shared success/failure delivery, failure recovery, ignored replacement work, cancellation fallback, queued caller settlement, restart-after-cancel, and pending-interrupt replacement interleaving are covered; real Effect Scope/Fiber interruption, shell states, lifecycle callbacks, and scoped cleanup remain deferred", "EffectSmoke.runner"]],
    ["effect/run-service.test.ts", ["partial", "src/opencodehx/effect/RuntimeMemo.hx; src/opencodehx/effect/RunServiceRuntime.hx; src/opencodehx/smoke/EffectSmoke.hx; docs/effect-runtime-parity.md", "memoized shared dependency, Promise-backed service execution, typed exits including synchronous throws, callback success/failure delivery including synchronous callback action failures, and deterministic fork success/failure delivery including synchronous fork action failures, interruption, late-success suppression, and late-failure suppression are covered; full Effect ManagedRuntime/Layer/Context.Service and real Fiber semantics remain deferred", "EffectSmoke.runServiceMemoMap/runServiceAsync"]],
    ...
    ["workspace/workspace-restore.test.ts", ["partial", "src/opencodehx/controlplane/WorkspaceRestoreRuntime.hx; src/opencodehx/smoke/ControlPlaneSmoke.hx; docs/project-runtime-parity.md", "local/remote replay, batching/progress, and zero-input session-update append behavior are covered; full database/AppRuntime/Workspace service integration and persisted session workspace updates remain deferred", "ControlPlaneSmoke.workspaceRestoreRemote/workspaceRestoreLocal/workspaceRestoreEmptyLocal", "opencodehx-000.11"]],
    ...
    ["tool/write.test.ts", ["partial", "src/opencodehx/tool/WriteTool.hx; src/opencodehx/smoke/ToolSmoke.hx; docs/core-file-tools-port.md", "write creation, overwrite metadata, JSON/binary-safe/empty/multiline/CRLF content preservation, relative title output, diff/filediff metadata, BOM preservation including formatter-strip restore, file.edited plus file.watcher add/change publication, focused LSP diagnostic output/metadata, and external-directory write permission shape are covered; real LSP service/process integration and full async Format service integration remain deferred", "ToolSmoke.writeExec"]],
    [
      "tool/edit.test.ts",
      [
        "partial",
        "src/opencodehx/tool/EditTool.hx; src/opencodehx/smoke/ToolSmoke.hx; docs/core-file-tools-port.md",
        "representative exact/multiline/replace-all/multiple-match behavior, identical-input and directory-path failures, filediff stats, tolerant fallback behavior, CRLF/BOM preservation including formatter-strip restore, file.edited plus file.watcher add/change publication, focused LSP diagnostic output/metadata, and external-directory edit permission shape are covered; full upstream async Format service and real LSP service/process integration remain deferred",
        "ToolSmoke.editExec",
      ],
    ],
    [
      ...
      [
        ...
        "representative add/update/delete/move, insert-only hunks, BOM preservation including formatter-strip restore, file.edited plus file.watcher add/change/unlink publication, focused LSP diagnostic output/metadata for non-delete targets, EOF, context disambiguation, heredoc with/without cat, whitespace/Unicode matching, malformed/delete-target failures, no-side-effect behavior, and external-directory hunk/move-target permission shape are covered; full upstream async Format service, Effect runtime wiring, and real LSP service/process integration remain deferred",
    ...
    [
      ...
      [
        ...
        "focused snapshot runtime covers deterministic/repeated track hashes, project state isolation, secondary worktree patch/revert/diff isolation, patch detection for added/modified/deleted files including special ASCII filenames, added/modified/deleted Unicode filenames and nested Unicode paths, long filenames, hidden files, and file/nested-directory/circular symlink patch plus symlink revert lifecycle evidence on capable hosts, under-limit added-file tracking, chmod-only no-op changes, revert restore/delete behavior including recreated-file, nested added files, empty parent directories after added-file removal, non-existent requested files, overlapping first-patch, same-hash file/directory patch order, large mixed batch, and added/modified binary cases, focused restore of snapshot files while preserving new files and replacing file/directory path blockers, empty-directory no-op, invalid-hash empty patch, large added-file skip/stable hash, preexisting and newly added .gitignore filtering, .git/info/exclude rules, and global core.excludesFile rules, simple diff/diffFull evidence including no-change, deterministic order, added/modified/deleted and multiline-added text patch metadata, mixed and multiple add/delete entries, large interleaved text/binary mixed diffs, whitespace edits, added/deleted/modified statuses, grow-only/trim-only text churn counts, and binary diffFull empty-patch/zero-churn shape; persistent separate Git-dir lifecycle, restore cleanup/removal semantics, richer text diffFull patch metadata, broader symlink edge cases, cleanup/prune, and concurrency behavior remain deferred",
        "SnapshotSmoke",
        m11Owners.foundation,
      ],
    ],
    ...
    ["installation/installation.test.ts", ["partial", "src/opencodehx/smoke/ProjectRuntimeSmoke.hx; docs/project-runtime-parity.md", "installation method detection for npm/yarn/pnpm/bun/brew/scoop/choco/POSIX-and-Windows-curl/unknown, latest-version lookup for unknown-method GitHub fallback plus GitHub/npm/pnpm/bun registry normalization and brew/scoop/choco shapes, release-type classification including v-prefixed tags, curl install-script upgrade planning, npm/pnpm/Bun/Scoop/Chocolatey upgrade command planning with success version probes, Homebrew core/tap upgrade planning plus empty-repo skip and tap/pull failure containment, Chocolatey elevated-shell failure messaging, unknown-method upgrade failure, and package-manager uninstall command planning plus no-op methods are covered with typed injected HTTP/process seams; real package-manager side effects remain deferred", "ProjectRuntimeSmoke installationRuntime fixture", m11Owners.foundation]],
    ["npm.test.ts", ["partial", "src/opencodehx/smoke/ProjectRuntimeSmoke.hx; docs/project-runtime-parity.md", "package spec sanitizing, package-name parsing including scoped/unscoped npm: alias prefixes, package cache add/install/which/outdated semantics including null entrypoint resolver behavior, injected/default writability checks, empty reify failure, which add failure containment, no-input/versioned/bare install add specs, requested lock-hit skip, requested lock-miss reify, optional dependency lock-hit skip, missing/malformed package-lock reify, malformed registry JSON, missing latest metadata, string/object bin selection with single-key and first-key fallback plus no-metadata file fallback, peer dependency lock checks, comparator edge plus whole-range and minor/patch wildcard satisfaction/escape, zero-major caret ranges, and tilde/disjunction ranges, installation package-manager discovery, uninstall command planning, and config dependency bootstrap are covered through typed injected seams; live package-manager side effects remain deferred", "ProjectRuntimeSmoke npmSanitize/npmRuntime/installationRuntime and ConfigSmoke dependencyBootstrap fixtures", m11Owners.foundation]],
