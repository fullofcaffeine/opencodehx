...
```text
[######################################--] 95% (182/191 non-epic port beads closed)
```
...
- Local no-network live OpenAI-compatible fixtures prove successful streaming, provider-error persistence/export, registry tool schema advertisement, bounded continuation calls, and model-emitted `read`, `write`, `edit`, `apply_patch`, and `bash` tool calls with persisted/exported tool parts and workspace side effects.
- The npm package smoke installs the packed binary globally in a temporary prefix and repeats the installed `run --dir`, `run --file`, mock/live run, live tool-call, persistence/export/resume/continue/fork, TUI scaffold, and `serve` health/SSE/session/PTY workflows.
