...
```text
[#######################################-] 97% (267/276 non-epic port beads closed)
```
...
- The generated CLI has deterministic fake-provider `run`, credential-free mock AI SDK `run`, local live AI SDK `run --model provider/model`, plain config-model/default-agent live `run`, `run --file`, recovered text/file/tool/media/compaction/subtask/interrupted-output history prompts, default and `OPENCODE_DB` session persistence, `run --session`, `run --continue`, `run --fork`, and non-interactive `export <sessionID>`.
...
The CLI still defaults to the deterministic fake provider when no local config model or default-agent model is present. The live path is real but intentionally thin: it has credential-free local provider evidence, side-effecting tool-loop evidence, provider-aware request options and prompt transforms, selected/default agent request assembly, retryable stream-error intent recording, upstream-shaped provider/environment/skills/instruction system prompts including remote instruction URLs, nearby read-tool instruction reminders with per-message dedupe and recovered loaded-history suppression, and recovered text/file/tool/media/compaction/subtask/interrupted-output prompt history. Full agentic chat parity still needs upstream message-history edges such as provider metadata and provider-transform history cases, cancellation and retry loop scheduling, server-backed session orchestration, richer interactive permission prompting UX, reminder prompt services, broader provider loading, live plugin installs/imports/auth/adaptors, real MCP/ACP transports and OAuth, real LSP process/download flows, live TUI behavior beyond the scaffold, and periodic upstream drift runs using the documented rebase procedure.
