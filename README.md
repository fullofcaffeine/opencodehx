...
```text
[#######################################-] 97% (270/279 non-epic port beads closed)
```
...
The CLI still defaults to the deterministic fake provider when no local config model or default-agent model is present. The live path is real but intentionally thin: it has credential-free local provider evidence, side-effecting tool-loop evidence, provider-aware request options and prompt transforms, selected/default agent request assembly, retryable stream-error intent recording, live AI SDK stream/continuation cancellation evidence, upstream-shaped provider/environment/skills/instruction system prompts including remote instruction URLs, nearby read-tool instruction reminders with per-message dedupe and recovered loaded-history suppression, and recovered text/file/tool/media/provider-metadata/provider-transform/compaction/subtask/interrupted-output prompt history. Full agentic chat parity still needs retry loop scheduling, server-backed session orchestration and cancellation routing, richer interactive permission prompting UX, reminder prompt services, broader provider loading, live plugin installs/imports/auth/adaptors, real MCP/ACP transports and OAuth, real LSP process/download flows, live TUI behavior beyond the scaffold, and periodic upstream drift runs using the documented rebase procedure.
