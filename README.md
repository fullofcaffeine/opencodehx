...
```text
[######################################--] 95% (185/194 non-epic port beads closed)
```
...
- Local no-network live OpenAI-compatible fixtures prove successful streaming, provider-error persistence/export, registry tool schema advertisement, bounded continuation calls, model-emitted `read`, `write`, `edit`, `apply_patch`, and `bash` tool calls, a write-then-read multi-step tool chain, config-denied live tool calls, and `--dangerously-skip-permissions` approval for config-asked writes with persisted/exported tool parts and workspace side effects.
