...
```text
[######################################--] 95% (197/207 non-epic port beads closed)
```
...
- The runtime foundation includes config, auth discovery, permissions, parser-backed bash permissions, file/search tools, the standalone patch runtime, provider registry and SDK factory smokes, models.dev cache/fetch, storage, sessions, server routes, SDK-compatible create/list/resume/event flow, MCP/ACP/LSP/plugin minimum seams, PTY lifecycle/WebSocket replay, native file-watcher evidence, performance baselines, checked artifact macros, and TUI scaffold/transcript/dialog slices.
