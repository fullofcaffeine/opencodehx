...
Current beta packaging expectations include a local `npm pack` plus temporary global-install smoke for the generated `opencodehx` bin, including deterministic `run`, deterministic `run --dir`, mock AI SDK `run --mock-ai-sdk --dir`, and installed `serve` `/health`; keep that gate in sync with package metadata, CLI workspace resolution, or CLI server startup changes before the next generated release notes are prepared.
