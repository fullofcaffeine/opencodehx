...
Current beta packaging expectations include a local `npm pack` plus temporary global-install smoke for the generated `opencodehx` bin, including deterministic `run` and installed `serve` `/health`; keep that gate in sync with package metadata or CLI server startup changes before the next generated release notes are prepared.
